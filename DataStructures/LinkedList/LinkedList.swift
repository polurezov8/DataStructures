...
public final class LinkedList<T> {
  ...
  public func node(at index: Int) -> Node {
    ...
    assert(index >= .zero, "index must be greater or equal to 0")
...
extension LinkedList: Collection {
  ...
  public var startIndex: Index {
    get {
      return LinkedListIndex<T>(node: head, tag: .zero)
    }
  }
...
public struct LinkedListIndex<T>: Comparable {
  ...
  public static func == <T>(lhs: LinkedListIndex<T>, rhs: LinkedListIndex<T>) -> Bool { lhs.tag == rhs.tag }
  public static func <<T>(lhs: LinkedListIndex<T>, rhs: LinkedListIndex<T>) -> Bool { lhs.tag < rhs.tag }
}
...
class LinkedListTest: XCTestCase {
  ...
  func testInsertAtIndex() {
    ...
    XCTAssertFalse(previous === node)
    XCTAssertFalse(next === node)
    XCTAssertTrue(previous.next === node)
  ...
  func testRemoveAtIndex() {
    ...
    XCTAssertTrue(previous.next === node)
    XCTAssertTrue(node.previous === previous)
  ...
  func testRemoveLast() {
    ...
    XCTAssertTrue(list.last === previous)
    XCTAssertEqual(nodeCount - 1, list.count)
  }
